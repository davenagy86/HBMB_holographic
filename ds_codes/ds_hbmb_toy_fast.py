# hbmb_ds_toy_v4.py
# dS toy: local-horizon eigenmode reconstruction on S^2 (HBMB cutoff)
...
def main():
    ...
    plt.ylabel("Error")
    plt.title("dS toy: reconstruction error vs cutoff (FAST v4)")
    plt.legend()
    p1 = os.path.join(out_dir, "ds_toy_error_vs_lmax_fast_v4.png")
    ...
    plt.title("dS toy: spectral coverage vs cutoff (FAST v4)")
    plt.legend()
    p2 = os.path.join(out_dir, "ds_toy_coverage_vs_lmax_fast_v4.png")
    ...
    for lm in lmax_maps:
        ...
        fig.suptitle("dS toy: angular reconstruction on the local horizon (FAST v4)")
        p = os.path.join(out_dir, f"ds_toy_maps_lmax_{lm}_fast_v4.png")
